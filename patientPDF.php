...
$patientId = isset($_GET['id']) ? (int)$_GET['id'] : 0; // Get the ID from URL, default to 0 if not set

// Query to fetch patient information based on the ID
$query = "SELECT * FROM patients WHERE id = $patientId";
$result = $mysqli->query($query);

// Check if the patient data is retrieved
if ($result && $row = $result->fetch_assoc()) {
    ...
    $patient_is = $row['patient_is'];
    $dswd_member = $row['dswd_member'];
    $philhealth_member = $row['philhealth_member'];
    $ekonsulta_member = $row['ekonsulta_member'];
    $phic_id = $row['phic_id'];
    ...
    $firstName = $middleInitial = $lastName = ''; // Default values if no patient data found
}
...
$pdf->Cell(50, 5, "$lastName,", 0, 0, 'L'); // Last Name

// First Name
$pdf->SetX(80); // Move the cursor after the last name
$pdf->Cell(50, 5, "$firstName", 0, 0, 'L'); // First Name

// Middle Initial
$pdf->SetX(155); // Move the cursor after the first name
$pdf->Cell(0, 5, "$middleInitial", 0, 1, 'L'); // Middle Initial
...
$pdf->SetX(30);
$pdf->Cell(0, 5, "$address", 0, 0, 'L'); // Address

$pdf->SetX(75); // Move the cursor after the Address
$pdf->Cell(0, 5, "Date of Birth: ", 0, 0, 'L'); // Birth Date

$pdf->SetX(100); // Move the cursor after the date of birth
$pdf->Cell(0, 5, "$birthday", 0, 0, 'L'); // Birthday

$pdf->SetX(140); // Move the cursor after the birthday
$pdf->Cell(0, 5, "Birth Place: ", 0, 0, 'L'); // Place of Birth

$pdf->SetX(160); // Move the cursor after the Birth Place
$pdf->Cell(0, 5, "$birthPlace", 0, 0, 'L'); // Birth Place
...
// HEAD OF FAMILY
$pdf->SetX(38);
$pdf->Cell(0, 5, ($patient_is == "Head" ? "_/_" : "___"), 0, 0, 'L');
$pdf->SetX(43);
$pdf->Cell(0, 5, "HEAD OF FAMILY", 0, 0, 'L');

// WIFE OF HEAD OF FAMILY
$pdf->SetX(65);
$pdf->Cell(0, 5, ($patient_is == "Wife" ? "_/_" : "___"), 0, 0, 'L');
$pdf->SetX(70);
$pdf->Cell(0, 5, "WIFE OF HEAD OF FAMILY", 0, 0, 'L');

// FATHER
$pdf->SetX(105);
$pdf->Cell(0, 5, ($patient_is == "Father" ? "_/_" : "___"), 0, 0, 'L');
$pdf->SetX(110);
$pdf->Cell(0, 5, "FATHER", 0, 0, 'L');

// MOTHER
$pdf->SetX(123);
$pdf->Cell(0, 5, ($patient_is == "Mother" ? "_/_" : "___"), 0, 0, 'L');
$pdf->SetX(128);
$pdf->Cell(0, 5, "MOTHER", 0, 0, 'L');

// DAUGHTER
$pdf->SetX(141);
$pdf->Cell(0, 5, ($patient_is == "Daughter" ? "_/_" : "___"), 0, 0, 'L');
$pdf->SetX(146);
$pdf->Cell(0, 5, "DAUGHTER", 0, 0, 'L');

// SON
$pdf->SetX(162);
$pdf->Cell(0, 5, ($patient_is == "Son" ? "_/_" : "___"), 0, 0, 'L');
$pdf->SetX(167);
...
$pdf->SetX(91);
$pdf->Cell(0, 5, ($philhealth_category == "indigent" ? "_/_" : "___"), 0, 0, 'L');
...
// Position for the first row (BP)
$pdf->SetXY(10, 95); // Position for the top-left corner of the table
$pdf->Cell(30, 5, "$consult_date", 1, 0, 'C');
$pdf->Cell(40, 5, "$subjective", 0, 0, 'C'); 
$pdf->Cell(40, 5, 'BP: ' . $bp . ' mmHg', 1, 0, 'L'); // Insert database value here
$pdf->Cell(40, 5, "$assessment", 1, 0, 'C');
$pdf->Cell(40, 5, "$plan", 1, 1, 'C');

// Position for the second row (HR)
$pdf->SetXY(10, 100);
$pdf->Cell(30, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, 'HR: ' . $hr. ' bpm', 1, 0, 'L'); // Insert database value here
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 1, 'C');

// Position for the third row (RR)
$pdf->SetXY(10, 105);
$pdf->Cell(30, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, 'RR: ' . $rr . ' bpm', 1, 0, 'L'); // Insert database value here
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 1, 'C');

// Position for the fourth row (TEMP)
$pdf->SetXY(10, 110);
$pdf->Cell(30, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, 'TEMP: ' . $tr . ' °C', 1, 0, 'L'); // Insert database value here
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 1, 'C');

// Position for the fifth row (HT)
$pdf->SetXY(10, 115);
$pdf->Cell(30, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, 'HT: ' . $ht . ' cm', 1, 0, 'L'); // Insert database value here
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 1, 'C');

// Position for the sixth row (WT)
$pdf->SetXY(10, 120);
$pdf->Cell(30, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, 'WT: ' . $wt . ' kg', 1, 0, 'L'); // Insert database value here
$pdf->Cell(40, 5, '', 1, 0, 'C');
$pdf->Cell(40, 5, '', 1, 1, 'C');

$yPosition = 120;  // Starting Y-coordinate
